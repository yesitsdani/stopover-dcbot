module.exports = {
    ...
    async execute(client, message, args) {
        if (!args[0]) return await message.reply(`Use the arguments: \`teststart\`, \`testend\`, ⚠️\`start\`, ⚠️\`end\`, ⚠️\`maze\``);
        const valid = ['teststart', 'testend', 'start', 'end', 'maze'];
        if (!valid.includes(args[0].toLowerCase())) return await message.reply(`Use the arguments: \`teststart\`, \`testend\`, ⚠️\`start\`, ⚠️\`end\``);
        ...
        if (args[0].toLowerCase() == "end") rolesToAdd = ['1532389210827391066', '1531529841717809232'];
